...
def get_pie_chart(value):
    ...
    if value == 'All Sites':
        ...
        fltr_df=spacex_df[spacex_df['Launch Site']==value].groupby(['Launch Site','class']). \
        size().reset_index(name='class_count')
        fig =px.pie(fltr_df, values='class_count',names="class",
...
def get_scatter_chart(value1,value2):
    fltr_df2_1=spacex_df[(spacex_df['Payload Mass (kg)'] >value2[0]) & \
        (spacex_df['Payload Mass (kg)'] < value2[1])]
    
    if value1 == 'All Sites':
        fig = px.scatter(fltr_df2_1,x="Payload Mass (kg)",y="class",color="Booster Version Category",
        title='Correlation between Payload and Success for all Sites'
        )
        return fig
    else:
        fltr_df2_2=fltr_df2_1[fltr_df2_1['Launch Site']==value1]
        fig=px.scatter(fltr_df2_2,x='Payload Mass (kg)',y='class',color='Booster Version Category',
